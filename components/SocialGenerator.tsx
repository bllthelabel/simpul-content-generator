...
import { generateBrandText, generateBrandImage } from '../services/geminiService';
...
export default function SocialGenerator() {
  ...
  const handleGenerate = async (manualCaption?: string | unknown) => {
    ...
    try {
      ...
      const result = await generateBrandText(textToProcess, selectedLayout, isCarousel);
      if (Array.isArray(result)) {
        const newSlides = result.map((content) => ({
            ...DEFAULT_CONTENT, 
            ...content,         
        ...
        setSlides(prev => {
            const newSingle = { ...prev[0], ...result };
            return [newSingle];
        });
        setActiveSlideIndex(0);
      }
      setVisibility({ tagline: true, headline: true, subtext: true, body: true, cta: true });
                ...
                <div className="grid grid-cols-3 gap-6">
                    ...
                    <div className="space-y-1 h-full">
                        ...
                        <textarea 
                            ...
                            className="w-full h-[calc(100%-1.5rem)] bg-slate-800 border border-slate-700 rounded px-3 py-2 text-sm text-white focus:border-brand-primary focus:ring-0 resize-none"
                        />
                    </div>
                </div>
            </div>
